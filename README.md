...
| 場所 | 内容 |
|------|------|
| 上段 | カメラ映像＋ランドマーク．人数（1人 / 複数人）の切り替え，見せ方（生映像 / skeleton / overlay / 非表示）の切り替え，顔 / 体（pose）/ 左手 / 右手の表示チェックボックス，カメラ選択のプルダウン |
| ボタン列 | Record（⌘R）/ Switch（次のカメラへ）/ Play（最新の録画を映像エリアで再生．もう一度押すとカメラ映像に戻る）/ 選んで再生…（過去の録画をファイル選択で再生）．再生中は録画全体の波形を出し，赤い線がシークバーと同じ位置を動く．グラフ上でトラックパッドのピンチまたはマウスホイールの上下でカーソル位置を中心に横軸をズーム（最小 0.2 秒），横スクロールで移動，「全体」で戻す．再生位置が表示範囲から外れると範囲が追従する / Reveal（保存先を Finder で開く）/ 動画を処理…（動画ファイルを読み込んで抽出・書き出し・再生）/ 保存先…（出力フォルダを選ぶ） |
| 下段 | 波形グラフ（チェックボックス列との境目を左右にドラッグすると横幅を調整できる）．「上半身」「手腕」の切り替えボタンでモードを選び，グラフ単位・系列単位のチェックボックスで表示 / 非表示を切り替え |
...
| 複数人 | YOLO（yolo11n）で人物の枠を検出・追跡し，人物ごとに切り出して Holistic Landmarker を適用（最大4人） | 描画は全員．グラフと face / hand / pose の CSV / JSON は画面中央に最も近い人（枠の中心が画面中央に近い人） |

複数人モードでもランドマークの精度と出力形式は1人モードと同じです（33点＋ワールド座標，顔478点，手21点）．
他の人物は半透明で描き，`json/{stem}/{stem}_people.json` に全員分のランドマーク（先頭が中央の人物）も保存します．

- 後ろにいる人（枠の高さが画面の 25% 未満）と信頼度の低い枠（0.4 未満）は検出対象から除きます．
  しきい値は `landmarker/landmarker.py` の `MultiDetector` の定数で変えられます．
- 推論は人数に比例して遅くなります（2人で 10 fps 前後）．
- YOLO の実行には Ultralytics と PyTorch が必要で，初回の `uv sync` で 1 GB 程度の依存関係を取得します．
  YOLO の重み（約 5 MB）は初回の切り替え時に自動でダウンロードされます．
- **Ultralytics は AGPL-3.0 ライセンス**です．研究利用は問題ありませんが，配布・商用利用の際は条件を確認してください．
- モードの設定は次回起動時も保持されます．
...
```
results_data/
├── csv/live_20260903_120000/live_20260903_120000_{face,hand,pose,metrics}.csv
...
`{stem}_metrics.csv` はグラフ用の指標（頭・体の向き，手首の位置・速度，前腕の角度など）で，
再生時に波形を出すために使います．列名は指標の識別子で，値が無いフレームは空欄です．
...
「動画を処理…」で動画ファイルを選ぶと，カメラを一時停止して，カメラと同じ流れで全フレームを推論・描画し，
保存先に録画と同じ構成（`csv` / `json` / `video_raw` / `video_overlay` / `video_skeleton`）で書き出します．
ファイル名（stem）は動画のファイル名です．同名の出力があれば上書きします．
処理中は映像エリアに進み具合が出て，映像とグラフも処理中のフレームに合わせて更新されます．
完了すると生成した overlay 動画と波形の再生に自動で切り替わります．「中止」で途中でやめられます（途中までの出力は残ります）．

先頭の 1 フレームは出力に含めません（録画サイズを決めるために使います）．
