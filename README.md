...
| ボタン列 | Record（⌘R）/ Switch（次のカメラへ）/ Play（最新の録画を映像エリアで再生．もう一度押すとカメラ映像に戻る）/ 選んで再生…（過去の録画をファイル選択で再生）．再生中と動画処理中はカメラのキャプチャを止めて負荷を下げる（Live に戻すと再開）．再生中は録画全体の波形を出し，赤い線がシークバーと同じ位置を動く．グラフ上でトラックパッドのピンチまたはマウスホイールの上下でカーソル位置を中心に横軸をズーム（最小 0.2 秒），横スクロールで移動，「全体」で戻す．再生位置が表示範囲から外れると範囲が追従する / Reveal（保存先を Finder で開く）/ 動画を処理…（動画ファイルを読み込んで抽出・書き出し・再生）/ 保存先…（出力フォルダを選ぶ） |
...
#### 複数人モードの既知の制約（検証が不十分な点）

複数人モードは，2本の1人用動画を横に並べた合成動画でしか動作確認していません．
実際に複数人を撮影した映像では次の問題が残っている可能性があります．

- **人同士の重なり**：枠が重なると切り出し画像に他の人の体が入り，Holistic が別の人の手や顔を拾うことがある
- **「中央の人」の入れ替わり**：毎フレーム枠の中心で選び直すため，2人が画面中央から同じくらいの距離にいるとグラフの対象が入れ替わる
- **一時的な見失い**：YOLO が枠を見失うと追跡 ID が変わり，その人の Holistic が作り直されて数フレーム途切れる
- **切り出しの解像度**：人が小さく写ると切り出し後の画素が少なく，手や顔の精度が1人モードより落ちる
- **速度**：人数に比例して遅くなる（2人で 10 fps 前後）

複数人で撮影した映像での検証結果に応じて，追跡 ID による対象の固定，しきい値の調整，切り出し余白の見直しを行う予定です．
