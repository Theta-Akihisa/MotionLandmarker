...
| ボタン列 | Record（⌘R）/ Switch（次のカメラへ）/ Play（最新の録画を映像エリアで再生．もう一度押すとカメラ映像に戻る）/ 選んで再生…（過去の録画をファイル選択で再生）．再生中は録画全体の波形を出し，赤い線がシークバーと同じ位置を動く / Reveal（保存先を Finder で開く）/ 動画を処理…（動画ファイルを読み込んで抽出・書き出し・再生）/ 保存先…（出力フォルダを選ぶ） |
